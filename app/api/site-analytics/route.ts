...
import type { PaymentPayload, Signer } from 'x402/types';
import { getServerWalletClient } from '@/lib/wallet';
import { BASE_SEPOLIA_USDC } from '@/lib/x402-config';
...
export async function GET(request: NextRequest) {
  ...
  if (!paymentHeader) {
    ...
    const paymentRequirements = {
      ...
      info: {
        siteName: 'DevNews',
        price: '$0.01 USDC',
        description: 'Get detailed traffic metrics, demographics, and ad spot performance data'
      }
    };
  ...
  try {
    ...
    const paymentRequirements = {
      ...
      asset: BASE_SEPOLIA_USDC as `0x${string}`,
    ...
    if (!verifyResult.isValid) {
      console.log(`❌ [analytics] Payment verification failed: ${verifyResult.invalidReason}`);
    ...
    if (!settleResult.success) {
      console.log(`❌ [analytics] Settlement failed: ${settleResult.errorReason}`);
      return NextResponse.json(
    ...
    console.log(`✅ [analytics] Payment settled! Tx: ${settleResult.transaction}`);
  ...
  const analytics = {
    site: {
      ...
      description: 'Aggregated technology news, tutorials, and insights for software engineers, DevOps professionals, and tech leaders.',
    ...
    demographics: {
      ...
      audienceType: 'Software Engineers (48%), DevOps Engineers (23%), Tech Leaders (18%), Data Scientists (8%), Other (3%)',
